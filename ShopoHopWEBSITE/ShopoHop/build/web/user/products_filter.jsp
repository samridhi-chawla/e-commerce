...
<%
    ...
    if(search == null)
    {
        rs = stmt.executeQuery("select * from products where sub_category='" + sub_category + "' and p_offer_price<=" + price_below + " and p_offer_price>='"+price_above+"'  and ((p_mrp-p_offer_price)/p_mrp)*100>=" + discount);
    }
    else{
        rs = stmt.executeQuery("select * from products where p_name like '%" + search + "%' and p_offer_price<=" + price_below + " and p_offer_price>='"+price_above+"' and ((p_mrp-p_offer_price)/p_mrp)*100>=" + discount);
    }
    int flag = 0;
    while (rs.next()) {
        flag++;
